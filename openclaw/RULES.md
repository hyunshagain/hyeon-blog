...
**영구 메모리에 의존하지 말고, 작업을 시작할 때마다 clone된 저장소의 이 파일을 다시 읽는다.**

저장소가 규칙의 원본이다. 메모리 백엔드가 죽거나 인증이 만료돼도 규칙은 살아 있어야 한다.
실제로 2026-08-03 첫 테스트에서 메모리 의미 검색이 HTTP 401로 실패했다.
그때 규칙이 메모리에만 있었다면 규칙 없이 돌아갔을 것이다.
...
7. **검사 결과나 수치를 지어내지 않는다.**
   이 저장소의 품질 게이트는 **점수제가 아니다.** 8개 검사의 통과/차단이고,
   출력은 `통과: 차단 N건, 경고 M건` 형식뿐이다. "97/100" 같은 점수는 존재하지 않는다.
   검사 결과를 보고할 때는 **`npm run gate` 또는 CI 로그의 출력을 그대로 인용한다.**
   실행하지 않은 검사의 결과를 요약하거나 추정하지 않는다. 실행하지 않았으면
   "실행하지 않음"이라고 쓴다.

   보고서에서는 무해하지만 글 본문에서는 허위 사실이 된다. 같은 습관이다.
